...
class FourierClass(FourierWindow):
    ...
    def initUI(self):
        ...
        self.ax = self.fig.add_subplot(211,autoscale_on='True',title="Mean Frequency Amplitude vs Frequency")
    ...
    def calcStatistics(self,name):
        ...
        for filename in os.listdir(checkDirectory):
                if filename.endswith("t.npy"):
                    ...
                    for i in range(0,3):
                        fourier_val,freq = self.calcFFT(cur_data[i,0:cur_ctr],cur_time[i,0:cur_ctr])
                        j = 0
                        m = 0
                        while freq[m] < 0:
                            m = m + 1
