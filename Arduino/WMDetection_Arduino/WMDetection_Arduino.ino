...
void loop()
{
	...
	//Check only after about 4ms*20 = 80ms
        if(checkCtr == SAMPLE_SIZE)
        {
          //Calculate mean
          float mean = 0;
          ...
          for(int i = 0;i<SAMPLE_SIZE;i++)
          {
            std += ((float)rssi_arr[i] - mean)*((float) rssi_arr[i] - mean);
          }
          //Serial.print(std,2);
          //Serial.print(" ");
          val = std/SAMPLE_SIZE;
          //Serial.print(std,4);
          //Serial.println(" ");
          //val = abs(cur_rssi[XBEE_TRIG] - oldVal);
          checkCtr = 0;
          //oldVal = cur_rssi[XBEE_TRIG];
        }  
        else
       {
         rssi_arr[checkCtr] = cur_rssi[XBEE_TRIG]; 
         checkCtr++;
       }
       
        //Serial.println(val,2);
        if(val < THRESHOLD) checkAna = false;
	else checkAna = true;
